...
function [ er, pred, truth ] = test_cnn_on_one_img(no_chnls, cnn,data_file_name,lbl_file_name,para,U,S,avg,epsilon)
    ...
    [er,pred,truth] = slice_and_test(cnn,para,d,l,U,S,avg,epsilon,no_chnls);
end

function [er,pred,truth] = slice_and_test(cnn,para,img,img_lbl,U,S,avg,epsilon,no_chnls)
...
%     data = nan(para.img_m,para.img_n,rgb,arg_end-arg_start+1);
    ...
    for n_idx=1:para.img_n-para.win_n
        wins = zeros(para.win_m,para.win_n,no_chnls,para.img_m-para.win_m);
        ...
        [p, t] = my_cnntest(cnn, wins, win_lbls, no_chnls);
        pred(:,n_idx) = p;
        truth(:,n_idx) = t;
    end
